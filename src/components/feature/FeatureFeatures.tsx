...
const ICONS = [CreditCard, Layers, Zap, ShieldCheck, Globe, Users, Cpu, MapPin, Radio];
...
export const FeatureExplainer: React.FC<Props> = ({ data }) => {
...
export const FeatureFeaturesGrid: React.FC<Props> = ({ data }) => {
  const items = data.features_grid;
  const header = data.features_header;

  return (
    <section id="feature-features" className="bg-white py-10 sm:py-18 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Swiper
          ...
          pagination={{ clickable: true, el: '.feature-features-dots' }}
          autoplay={{ delay: 4500, disableOnInteraction: true, pauseOnMouseEnter: true }}
          className="feature-features-swiper"
        >
          {items.map((item, idx) => {
        ...
        {/* Dots — bottom center */}
        <div className="feature-features-dots flex items-center justify-center gap-2 mt-8" />

        <style>{`
          .feature-features-swiper .swiper-slide { height: auto; display: flex; }
          .feature-features-swiper .swiper-slide > div { flex: 1 1 auto; width: 100%; }
          .feature-features-dots .swiper-pagination-bullet {
            width: 8px; height: 8px; background: #cbd5e1; opacity: 1;
            border-radius: 9999px; transition: all 0.3s ease; margin: 0 !important;
          }
          .feature-features-dots .swiper-pagination-bullet-active {
