...
import type { FeaturePageData, FeatureCTA } from '../../data/features';
...
const CTAButton: React.FC<{ cta: FeatureCTA; onOpenDemo?: () => void }> = ({ cta, onOpenDemo }) => {
  if (cta.type === 'primary') {
    return (
      <CtaLink
        cta={cta}
...
export const FeatureHero: React.FC<Props> = ({ data, onOpenDemo }) => {
  const { hero } = data; 
  return (
