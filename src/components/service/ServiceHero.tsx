...
export const ServiceHero: React.FC<Props> = ({ data, onOpenDemo }) => {
  const { hero } = data;
  return (
    <section className="relative overflow-hidden bg-white text-slate-900 flex flex-col justify-between">
      ...
      <div className="relative z-10 flex-1 flex flex-col lg:flex-row items-center max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 w-full pt-10 sm:pt-14 gap-8 lg:gap-6">
          ...
          <div className="relative w-full rounded-3xl overflow-hidden">
            ...
            {/* <div className="absolute bottom-0 left-0 right-0 p-6 flex flex-col gap-2">
              <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-4 max-w-md">
                <p className="text-sm font-bold text-white mb-1">Field-ready operations</p>
                <p className="text-xs text-slate-200 leading-relaxed">{hero.hero_image.description}</p>
              </div>
            </div> */}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-10 opacity-60">
            {data.integrations_payment.supported_gateways.map((b) => (
              <div key={b} className="text-slate-700 font-bold text-base sm:text-lg">
                {b}
              </div>
            ))}
          </div>
